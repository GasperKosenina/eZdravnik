...
const pretvoriZahtevekVBesedilo = (zahtevek: Zahtevek): string => {
  let besedilo =
    "Pozdravljen. Obnasaj se kot osebni zdravnik, ki pomaga pacientu pri njegovih simpotmih. Pacient je " +
    (zahtevek.spol ? "moski" : "zenska") +
    " in se je rodil/la:  " +
    zahtevek.datum_rojstva +
    ".\n" +
    "Pacient ima naslednje simptome: " +
    zahtevek.opis_simptomov +
    ".\n" +
    "Pacient jemlje naslednja zdravila: " +
    (zahtevek.zdravila ? zahtevek.zdravila : "Ne jemlje zdravil") +
    ".\n" +
    "Pacient ima naslednje alergije: " +
    (zahtevek.alergije ? zahtevek.alergije : "Nima alergij") +
    ".\n" +
    "Pacient ima v druzini naslednje bolezni: " +
    (zahtevek.bolezni_v_druzini
      ? zahtevek.bolezni_v_druzini
      : "Nima bolezni v druzini") +
    ".\n" +
    "Pacient ima naslednji zivljenjski slog: " +
    (zahtevek.zivljenjski_slog
      ? zahtevek.zivljenjski_slog
      : "Pacient ni opisal zivljenskega sloga") +
    ".\n" +
    "Pacient ima naslednji dodatni kontekst: " +
    (zahtevek.dodatni_kontekst
      ? zahtevek.dodatni_kontekst
      : "Nima dodatnega konteksta") +
    ".\n" +
...
export const dodajZahtevekInOdgovorPodUporabnika = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    if (!zahtevek.datum_rojstva || !zahtevek.opis_simptomov) {
      res.status(400).json({
        error: "Zahtevek mora vsebovati spol, datum rojstva in opis simptomov",
      });
      return;
    }
    ...
    const besediloOdgovora: string =
      await posiljanjeBesedilaChatGPT(pretvorjenoBesedilo);
    if (!besediloOdgovora) {
